"""
finetune based on UniTS backbone with floc.
"""
import math
import torch
...
class CrossAttention(nn.Module):
    def __init__(
        ...
        if var_num is not None:
            ...
            torch.nn.init.normal_(self.template, std=0.02)
        self.var_num = var_num

    def forward(self, x, query=None):
        B, N, C = x.shape
        if query is not None:
            q = self.q(query).reshape(B, query.shape[1], self.num_heads, self.head_dim).permute(0, 2, 1, 3)
            q = self.q_norm(q)
            var_num = query.shape[1]
        else:
            q = self.q(self.template).reshape(1, self.var_num, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
            q = self.q_norm(q)
            q = q.repeat(B, 1, 1, 1)
...
class DynamicLinear(nn.Module):
    ...
    def forward(self, x, out_features):
        ...
        if in_features != self.weights.size(1) or out_features != self.weights.size(0):
            dynamic_weights = F.interpolate(dynamic_weights.unsqueeze(0).unsqueeze(0), size=(out_features, in_features - self.fixed_in), mode="bilinear", align_corners=False).squeeze(0).squeeze(0)
            if self.fixed_in != 0:
                fixed_weights = F.interpolate(fixed_weights.unsqueeze(0).unsqueeze(0), size=(out_features, self.fixed_in), mode="bilinear", align_corners=False).squeeze(0).squeeze(0)
        if out_features != self.weights.size(0):
            this_bias = F.interpolate(this_bias.unsqueeze(0).unsqueeze(0).unsqueeze(0), size=(1, out_features), mode="bilinear", align_corners=False).squeeze(0).squeeze(0).squeeze(0)
...
class DynamicLinearMlp(nn.Module):
    def __init__(
        ...
        self.fc1 = nn.Conv1d(in_features, hidden_features, 3, groups=group, bias=bias[0], padding=1)
        self.act = act_layer()
        self.drop1 = nn.Dropout(drop_probs[0])

        self.norm = norm_layer(hidden_features) if norm_layer is not None else nn.Identity()
        self.seq_fc = DynamicLinear(hidden_features // 4, hidden_features // 4, bias=bias[1], fixed_in=prefix_token_length)
        self.prompt_fc = DynamicLinear(hidden_features // 4, prefix_token_length, bias=bias[1], fixed_in=prefix_token_length)
...
class SeqAttention(nn.Module):
    ...
    def __init__(
        ...
        self.qkv = LoRALinear(args, dim, dim * 3, merge=True)
        self.q_norm = norm_layer(self.head_dim) if qk_norm else nn.Identity()
        self.k_norm = norm_layer(self.head_dim) if qk_norm else nn.Identity()
        self.attn_drop = nn.Dropout(attn_drop)
        # self.proj = nn.Linear(dim, dim)
        self.proj = LoRALinear(args, dim, dim, merge=True)
        self.proj_drop = nn.Dropout(proj_drop)
...
class BasicBlock(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.seq_att_block = SeqAttBlock(args=args, dim=dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_norm=qk_norm, attn_drop=attn_drop, init_values=init_values, proj_drop=proj_drop, drop_path=drop_path, norm_layer=norm_layer)

        self.var_att_block = VarAttBlock(args=args, dim=dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_norm=qk_norm, attn_drop=attn_drop, init_values=init_values, proj_drop=proj_drop, drop_path=drop_path, norm_layer=norm_layer)

        self.dynamic_mlp = MLPBlock(dim=dim, mlp_ratio=mlp_ratio, mlp_layer=DynamicLinearMlp, proj_drop=proj_drop, init_values=init_values, drop_path=drop_path, act_layer=act_layer, norm_layer=norm_layer, prefix_token_length=prefix_token_length)
...
class CLSHead(nn.Module):
    def __init__(self, d_model, head_dropout=0):
        ...
        self.mlp = MLPBlock(dim=d_mid, mlp_ratio=8, mlp_layer=Mlp, proj_drop=head_dropout, init_values=None, drop_path=0.0, act_layer=nn.GELU, norm_layer=nn.LayerNorm, prefix_token_length=None)
...
class ForecastHead(nn.Module):
    ...
    def forward(self, x_full, pred_len, token_len):
        ...
        x = x.permute(0, 2, 1)
        x = torch.nn.functional.fold(x, output_size=(pred_len, 1), kernel_size=(self.patch_len, 1), stride=(self.stride, 1))
        x = x.squeeze(dim=-1)
...
class base_Model(nn.Module):
    def __init__(self, configs, pretrain=False):
        ...
        configs_list = [
            [
                configs.dataset,
                {
                    ...
                    "seq_len": configs.window_size,
                    "step": configs.time_step,
                    "pred_len": 0,
                    "enc_in": configs.input_channels,
                    "dec_in": configs.input_channels,
                    "c_out": configs.input_channels,
                },
            ]
        ]
        if pretrain:
        ...
        for i in range(self.num_task):
            ...
            if dataset_name not in self.prompt_tokens:
                self.prompt_tokens[dataset_name] = nn.Parameter(torch.zeros(1, configs_list[i][1]["enc_in"], args.prompt_num, args.d_model))

                torch.nn.init.normal_(self.prompt_tokens[dataset_name], std=0.02)

                self.mask_tokens[dataset_name] = nn.Parameter(torch.zeros(1, configs_list[i][1]["enc_in"], 1, args.d_model))

            if configs_list[i][1]["task_name"] == "classification":
                self.category_tokens[task_data_name] = torch.zeros(1, configs_list[i][1]["enc_in"], configs_list[i][1]["num_class"], args.d_model)
                torch.nn.init.normal_(self.category_tokens[task_data_name], std=0.02)
                self.cls_tokens[task_data_name] = torch.zeros(1, configs_list[i][1]["enc_in"], 1, args.d_model)
                torch.nn.init.normal_(self.cls_tokens[task_data_name], std=0.02)
            if pretrain:
                self.cls_tokens[task_data_name] = torch.zeros(1, configs_list[i][1]["enc_in"], 1, args.d_model)
                torch.nn.init.normal_(self.cls_tokens[task_data_name], std=0.02)
        ...
        for i in range(self.num_task):
            ...
            if configs_list[i][1]["task_name"] == "classification":
                ...
                input_token_len = calculate_unfold_output_length(configs_list[i][1]["seq_len"] + padding, args.stride, args.patch_len)
                input_pad = args.stride * (input_token_len - 1) + args.patch_len - configs_list[i][1]["seq_len"]
                pred_token_len = calculate_unfold_output_length(configs_list[i][1]["pred_len"] - input_pad, args.stride, args.patch_len)
                real_len = configs_list[i][1]["seq_len"] + configs_list[i][1]["pred_len"]
                self.cls_nums[task_data_name] = [pred_token_len, configs_list[i][1]["pred_len"], real_len]
        ...
        # input processing
        self.patch_embeddings = PatchEmbedding(args.d_model, args.patch_len, args.stride, args.stride, args.dropout)
        ...
        self.blocks = nn.ModuleList([BasicBlock(args=args, dim=args.d_model, num_heads=args.n_heads, qkv_bias=False, qk_norm=False, mlp_ratio=8.0, proj_drop=args.dropout, attn_drop=0.0, drop_path=0.0, init_values=None, prefix_token_length=args.prompt_num) for l in range(args.e_layers)])

        # output processing
        self.cls_head = CLSHead(args.d_model, head_dropout=args.dropout)
        self.forecast_head = ForecastHead(args.d_model, args.patch_len, args.stride, args.stride, prefix_token_length=args.prompt_num, head_dropout=args.dropout)
        self.out_projection = MLPProjection(configs_list[0][1]["enc_in"], args.prompt_num, configs_list[0][1]["seq_len"], args.patch_len, args.d_model, args.hidden_size)

        if pretrain:
            self.pretrain_head = ForecastHead(args.d_model, args.patch_len, args.stride, args.stride, prefix_token_length=1, head_dropout=args.dropout)
    ...
    def prepare_prompt(self, x, n_vars, prefix_prompt, task_prompt, task_prompt_num, task_name=None, mask=None):
        ...
        if task_name == "forecast":
            ...
            init_mask_prompt = self.prompt2forecat(init_full_input.transpose(-1, -2), init_full_input.shape[2] - prefix_prompt.shape[2]).transpose(-1, -2)
            this_function_prompt = init_mask_prompt[:, :, -task_prompt_num:]
            x = torch.cat((this_prompt, x, this_function_prompt), dim=2)
            x[:, :, self.prompt_num :] = x[:, :, self.prompt_num :] + self.position_embedding(x[:, :, self.prompt_num :])
    ...
    def forecast(self, x, x_mark, task_id):
        ...
        x = self.prepare_prompt(x, n_vars, prefix_prompt, task_prompt, task_prompt_num, task_name="forecast")
    ...
    def classification(self, x, x_mark, task_id):
        ...
        x = self.prepare_prompt(x, n_vars, prefix_prompt, task_prompt, task_prompt_num, task_name="classification")
    ...
    def imputation(self, x, x_mark, mask, task_id):
        ...
        x = self.prepare_prompt(x, n_vars, prefix_prompt, task_prompt, None, mask=mask, task_name="imputation")
        seq_token_len = x.shape[-2] - prefix_prompt.shape[2]
        x = self.backbone(x, prefix_prompt.shape[2], seq_token_len)
    ...
    def pretraining(self, x, x_mark, task_id, enable_mask=False):
        ...
        if enable_mask:
            mask = self.choose_masking(x, self.right_prob, self.min_mask_ratio, self.max_mask_ratio)
        ...
        if enable_mask:
            mask_dec_out = self.forecast_head(x[:, :, :-1], seq_len + padding, seq_token_len)
            mask_dec_out = mask_dec_out[:, :seq_len]
            # De-Normalization from Non-stationary Transformer
            mask_dec_out = mask_dec_out * (stdev[:, 0, :].unsqueeze(1).repeat(1, mask_dec_out.shape[1], 1))
            mask_dec_out = mask_dec_out + (means[:, 0, :].unsqueeze(1).repeat(1, mask_dec_out.shape[1], 1))
            cls_dec_out = self.cls_head(x, return_feature=True)
            # detach grad of the forecasting on tokens
            fused_dec_out = torch.cat((cls_dec_out, x[:, :, self.prompt_num : -1].detach()), dim=2)
            cls_dec_out = self.pretrain_head(fused_dec_out, seq_len + padding, seq_token_len)
            cls_dec_out = cls_dec_out[:, :seq_len]
            cls_dec_out = cls_dec_out * (stdev[:, 0, :].unsqueeze(1).repeat(1, cls_dec_out.shape[1], 1))
            cls_dec_out = cls_dec_out + (means[:, 0, :].unsqueeze(1).repeat(1, cls_dec_out.shape[1], 1))
    ...
    def forward(self, x_enc, x_mark_enc, x_dec=None, x_mark_dec=None, mask=None, task_id=None, task_name=None, enable_mask=None):
        ...
        if task_name == "imputation":
            dec_out = self.imputation(x_enc, x_mark_enc, mask, task_id)
            return dec_out  # [B, L, D]
        ...
        if "pretrain" in task_name:
            dec_out = self.pretraining(x_enc, x_mark_enc, task_id, enable_mask=enable_mask)
            return dec_out
        return None

class MLPProjection(nn.Module):
    def __init__(self, variable_nums, prompt_num, seq_len, patch_len, d_m, hidden_size=64):
        super(MLPProjection, self).__init__()
        sequence_len = prompt_num + math.ceil(seq_len / patch_len)
        input_dim = variable_nums * sequence_len * d_m
        self.projection = nn.Sequential(
            nn.Linear(input_dim, input_dim // 2, bias=False),
            nn.BatchNorm1d(input_dim // 2),
            nn.ReLU(inplace=True),
            nn.Linear(input_dim // 2, hidden_size, bias=False),
        )
...
class LoRALinear(nn.Module):
    ...
    def __init__(self, args, in_features, out_features, merge=True):
        super(LoRALinear, self).__init__()
        self.in_features = in_features
        self.out_features = out_features
        self.merge = merge
        self.rank = args.lora_rank
        self.dropout_rate = args.lora_dropout = 0.5
