...
class BasicBlock(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.seq_att_block = SeqAttBlock(dim=dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_norm=qk_norm, attn_drop=attn_drop, init_values=init_values, proj_drop=proj_drop, drop_path=drop_path, norm_layer=norm_layer)

        self.var_att_block = VarAttBlock(dim=dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_norm=qk_norm, attn_drop=attn_drop, init_values=init_values, proj_drop=proj_drop, drop_path=drop_path, norm_layer=norm_layer)
...
class base_Model(nn.Module):
    ...
    def __init__(self, configs, pretrain=False):
        ...
        self.blocks = nn.ModuleList([BasicBlock(dim=args.d_model, num_heads=args.n_heads, qkv_bias=False, qk_norm=False, mlp_ratio=8.0, proj_drop=args.dropout, attn_drop=0.0, drop_path=0.0, init_values=None, prefix_token_length=args.prompt_num) for l in range(args.e_layers)])
